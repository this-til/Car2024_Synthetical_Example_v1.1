...
public class ConnectTransport {
    ...
    private void send() {
        ...
        directSend(new byte[]{0x55, (byte) TYPE, (byte) MAJOR, (byte) FIRST, (byte) SECOND, (byte) THRID, (byte) CHECKSUM, (byte) 0xBB});
    }


    final ByteBuffer bigByteBuffer = ByteBuffer.allocate(64);
    final ByteBuffer byteBuffer = ByteBuffer.allocate(16);

    /***
     * 发送一串自定义数据
     * @param data 源数据
     * @param useCheck 如果true表示启用校验
     * @param addHeaderAndTail 如果true将添加帧头和帧尾
     */
    public void sendCustomData(byte[] data, boolean useCheck, boolean addHeaderAndTail) {
        Log.d("test", "发送自定义数据");
        if (useCheck || addHeaderAndTail) {
            synchronized (bigByteBuffer) {
                if (addHeaderAndTail) {
                    bigByteBuffer.put((byte) 0x55);
                }
                bigByteBuffer.put(data);
                byte[] finalData = data;
                bigByteBuffer.put((byte) (IntStream.range(0, data.length).mapToLong(i -> finalData[i]).sum() % 255));
                if (addHeaderAndTail) {
                    bigByteBuffer.put((byte) 0xBB);
                }

                data = bigByteBuffer.array();
            }
        }
        directSend(data);
    }

    public void directSend(byte[] sbyte) {
     ...
     * 发送车牌识别结果
     */
    public void sendLicensePlate(String licensePlate) {
        if (licensePlate.length() != 6) {
            throw new IllegalArgumentException("license plate length must be 6");
        }
        synchronized (byteBuffer) {
            byteBuffer.put((byte) 0xA1);
            byteBuffer.put(licensePlate.getBytes(StandardCharsets.US_ASCII));
            sendCustomData(byteBuffer.array(), true, true);
            byteBuffer.clear();
        }
    }
     ...
     * @param vehicleType
     */
    public void sendVehicleType(byte vehicleType) {
        synchronized (byteBuffer) {
            byteBuffer.put((byte) 0xA2);
            byteBuffer.put(vehicleType);
            sendCustomData(byteBuffer.array(), true, true);
            byteBuffer.clear();
        }
    }
     ...
     */
    public void sendNumberOfPeople(byte numberOfPeople) {
        synchronized (byteBuffer) {
            byteBuffer.put((byte) 0xA3);
            byteBuffer.put(numberOfPeople);
            sendCustomData(byteBuffer.array(), true, true);
            byteBuffer.clear();
        }
    }
     ...
     * 发送交通灯
     */
    public void sendTrafficLight(byte trafficLight) {
        synchronized (byteBuffer) {
            byteBuffer.put((byte) 0xA4);
            byteBuffer.put(trafficLight);
            sendCustomData(byteBuffer.array(), true, true);
            byteBuffer.clear();
        }
    }
     ...
     * 发送二维码
     */
    public void sendTwoDimensionalCode(byte[] data) {
        synchronized (byteBuffer) {
    ...
    public void sendTwoDimensionalCode(String data) throws UnsupportedEncodingException {
        sendTwoDimensionalCode(data.getBytes("GBK"));
    }

    /***
     * 发送图像
     */
    public void sendGraph(byte[] data) {
        if (data.length > 6) {
            throw new IllegalArgumentException("data length must be 6");
        }
        synchronized (byteBuffer) {
