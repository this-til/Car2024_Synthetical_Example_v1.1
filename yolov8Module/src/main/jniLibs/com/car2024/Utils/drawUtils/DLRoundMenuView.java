package car.bkrc.com.car2024.Utils.drawUtils;
...
public class DLRoundMenuView extends View {
     ...
     * 设置接口
     */
    private OnMenuClickAllListener mMenuClickAllListener;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    if (mMenuClickAllListener != null) {
                        mMenuClickAllListener.OnMenuLongClick(onClickState, motionEvent);
                    }
                    break;
            }
        }
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        final int defaultCoreMenuNormalBackgroundColor = res.getColor(R.color.default_core_menu_normal_background_color, null);
        final int defaultCoreMenuStrokeColor = res.getColor(R.color.default_core_menu_stroke_color, null);
        final float defaultCoreMenuStrokeSize = res.getDimension(R.dimen.default_core_menu_stroke_size);
        final int defaultCoreMenuSelectedBackgroundColor = res.getColor(R.color.default_core_menu_selected_background_color, null);
        final Drawable defaultCoreMenuDrawable = ContextCompat.getDrawable(context, R.drawable.default_core_menu_drawable);
        ...
        final Drawable defaultRoundMenuDrawable = ContextCompat.getDrawable(context, R.drawable.default_round_menu_drawable);
        final boolean defaultIsDrawLineToCenter = res.getBoolean(R.bool.default_is_draw_line_to_center);
        final int defaultRoundMenuNormalBackgroundColor = res.getColor(R.color.default_round_menu_normal_background_color, null);
        final int defaultRoundMenuSelectedBackgroundColor = res.getColor(R.color.default_round_menu_selected_background_color, null);
        final int defaultRoundMenuStrokeColor = res.getColor(R.color.default_round_menu_stroke_color, null);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (System.currentTimeMillis() - mTouchTime < 400) {
                    if (mMenuClickAllListener != null) {
                        isClick = true;
                        mMenuClickAllListener.OnMenuClick(onClickState, motionEvent);
                    }
                } else {    //非点击事件一定是长按事件，代表松手了，直接停止小车。
     ...
     * 设置监听
     *
     * @param onMenuClickAllListener
     */
    public void setOnMenuClickAllListener(OnMenuClickAllListener onMenuClickAllListener) {
        this.mMenuClickAllListener = onMenuClickAllListener;
    }
