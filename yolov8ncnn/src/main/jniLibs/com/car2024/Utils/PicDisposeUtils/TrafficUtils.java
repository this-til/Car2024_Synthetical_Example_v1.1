...
public class TrafficUtils {
    ...
    public static void detectTrafficLightColor(int id) {
        int red = 0, yellow = 0, green = 0;
        String circleColor = "";
        Mat grayImage = new Mat();
        Mat mat = new Mat();
        ...
        // 循环直到识别到红黄绿三种颜色中的一种或达到最大迭代次数
        while ((circleColor.isEmpty() || circleColor.equals("未识别到交通灯")) && iteration < maxIterations) {
            Bitmap bitmap = LeftFragment.INSTANCE.getBitmap(); // 从 LeftFragment 中获取最新的 Bitmap
            // 将图像转换为灰度图像，以便只需执行一次
            Utils.bitmapToMat(bitmap, mat);
            Imgproc.cvtColor(mat, grayImage, Imgproc.COLOR_BGR2GRAY);
        ...
        if (circleColor.equals("红色")) {
            FirstActivity.Connect_Transport.traffic_control(id, 0x02, 0x01);
        } else if (circleColor.equals("黄色")) {
            FirstActivity.Connect_Transport.traffic_control(id, 0x02, 0x03);
        } else if (circleColor.equals("绿色")) {
            FirstActivity.Connect_Transport.traffic_control(id, 0x02, 0x02);
        } else {
            Log.d("TrafficLight", "未识别到交通灯或达到最大迭代次数");
        }
    }
